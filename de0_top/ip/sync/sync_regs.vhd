library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

entity sync_regs is
   generic (
      C_DWIDTH             : integer   := 32;
      C_NUM_REG            : integer   := 16
   );
   port (
      clk                  : in    std_logic;
      reset_n              : in    std_logic;
      read_n               : in    std_logic;
      write_n              : in    std_logic;
      address              : in    std_logic_vector(10 downto 0);
      readdata             : out   std_logic_vector (31 downto 0);
      writedata            : in    std_logic_vector (31 downto 0);
      cpu_DOUT             : out   std_logic_vector(31 downto 0);
      cpu_DIN              : in    std_logic_vector(31 downto 0);
      cpu_ADDR             : out   std_logic_vector(9 downto 0);
      cpu_RE               : out   std_logic;
      cpu_WE               : out   std_logic;
      sync_CONTROL         : out   std_logic_vector(31 downto 0);
      sync_INT_REQ         : in    std_logic_vector(4 downto 0);
      sync_INT_ACK         : out   std_logic_vector(4 downto 0);
      sync_STATUS          : in    std_logic_vector(31 downto 0);
      sync_ADR_BEG         : out   std_logic_vector(31 downto 0);
      sync_ADR_END         : out   std_logic_vector(31 downto 0);
      sync_PKT_CNT         : out   std_logic_vector(31 downto 0);
      sync_PKT_XFER        : out   std_logic_vector(7 downto 0);
      sync_DEBUG           : in    std_logic_vector(31 downto 0);
      sync_TEST_BIT        : out   std_logic
   );
end sync_regs;

architecture rtl of sync_regs is

--
-- CONSTANTS
--
constant C_FIFO_VERSION    : std_logic_vector(7 downto 0)  := X"0A";
constant C_FIFO_CONTROL    : std_logic_vector(31 downto 0) := X"00000000";

--
-- SIGNAL DECLARATIONS
--

signal wrCE                : std_logic_vector(C_NUM_REG-1 downto 0);
signal rdCE                : std_logic_vector(C_NUM_REG-1 downto 0);

--
-- MAIN CODE
--
begin

   --
   -- COMBINATORIAL OUTPUTS
   --

   -- Write Block RAM
   cpu_RE               <= '1' when (address(10)  = '1' and read_n  = '0') else '0';
   cpu_WE               <= '1' when (address(10)  = '1' and write_n = '0') else '0';
   cpu_ADDR             <= address(9 downto 0);
   cpu_DOUT             <= writedata;

   --
   -- READ/WRITE REGISTER STROBES
   --
   process (all) begin
      for i in 0 to wrCE'length-1 loop
         if (address(4 downto 0) = std_logic_vector(to_unsigned(i, 5)) and
                  address(10) = '0' and write_n = '0') then
            wrCE(i) <= '1';
         else
            wrCE(i) <= '0';
         end if;
      end loop;
      for i in 0 to rdCE'length-1 loop
         if (address(4 downto 0) = std_logic_vector(to_unsigned(i, 5)) and
                  address(10) = '0' and read_n = '0') then
            rdCE(i) <= '1';
         else
            rdCE(i) <= '0';
         end if;
      end loop;
    end process;

   --
   -- WRITE REGISTERS
   --
   process (all) begin
      if (reset_n = '0') then
         sync_CONTROL         <= C_FIFO_CONTROL;
         sync_ADR_BEG         <= (others => '0');
         sync_ADR_END         <= (others => '0');
         sync_PKT_CNT         <= (others => '0');
         sync_PKT_XFER        <= (others => '0');
         sync_INT_ACK         <= (others => '0');
         sync_TEST_BIT        <= '0';
      elsif (rising_edge(clk)) then
         if (wrCE(0) = '1') then
            sync_CONTROL      <= writedata;
         elsif (wrCE(2) = '1') then
            sync_INT_ACK      <= writedata(4 downto 0);
         elsif (wrCE(4) = '1') then
            sync_ADR_BEG      <= writedata;
         elsif (wrCE(5) = '1') then
            sync_ADR_END      <= writedata;
         elsif (wrCE(6) = '1') then
            sync_PKT_CNT      <= writedata;
         elsif (wrCE(7) = '1') then
            sync_PKT_XFER     <= writedata(7 downto 0);
         elsif (wrCE(8) = '1') then
            sync_TEST_BIT     <= writedata(0);
         else
            sync_CONTROL      <= sync_CONTROL;
            sync_INT_ACK      <= (others => '0');
            sync_ADR_BEG      <= sync_ADR_BEG;
            sync_ADR_END      <= sync_ADR_END;
            sync_PKT_CNT      <= sync_PKT_CNT;
            sync_PKT_XFER     <= sync_PKT_XFER;
            sync_TEST_BIT     <= sync_TEST_BIT;
         end if;
      end if;
   end process;

   --
   -- READ REGISTERS AND BLOCK RAM
   --
   process (all) begin
      if (rdCE(0) = '1') then
         readdata             <= sync_CONTROL;
      elsif (rdCE(1) = '1') then
         readdata             <= X"000000" & C_FIFO_VERSION;
      elsif (rdCE(2) = '1') then
         readdata             <= X"000000" & "000" & sync_INT_REQ;
      elsif (rdCE(3) = '1') then
         readdata             <= sync_STATUS;
      elsif (rdCE(4) = '1') then
         readdata             <= sync_ADR_BEG;
      elsif (rdCE(5) = '1') then
         readdata             <= sync_ADR_END;
      elsif (rdCE(6) = '1') then
         readdata             <= sync_PKT_CNT;
      elsif (rdCE(7) = '1') then
         readdata             <= X"000000" & sync_PKT_XFER;
      elsif (rdCE(8) = '1') then
         readdata             <= X"0000000" & "000" & sync_TEST_BIT;
      elsif (rdCE(9) = '1') then
         readdata             <= sync_DEBUG;
      --
      -- READ BLOCK RAM
      --
      elsif (address(10) = '1' and read_n = '0') then
         readdata             <= cpu_DIN;
      else
         readdata             <= (others => '0');
      end if;
   end process;

end rtl;
